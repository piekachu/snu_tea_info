...
| `status` | 모집 상태 (아래 목록 참고) | `"recruiting"` |
...
| 값 | 표시 | 설명 |
|---|---|---|
| `recruiting` | 모집중 | 신청을 받고 있음 |
| `upcoming` | 모집예정 | 아직 신청이 열리지 않음 |
| `closed` | 마감 | 신청 종료 (행사 날짜가 지나면 자동으로 마감 처리됨) |
...
| `applicationDue` | 신청 마감일 (기본값: 행사일 3일 전) | `"2026-08-29"` |
...
- 행사 날짜가 지나면 `status`와 무관하게 자동으로 **마감** 처리됩니다.
- `applicationDue`를 지정하지 않으면 행사일 **3일 전**이 마감일로 자동 적용됩니다.
